...
# Number of subprocesses to fork to.
# Set to 1 to force sequential execution.
# Set to None to use one process per available CPU.
NUM_PROCESSES = None
...
# Small shim to call parse_csv from a 2-tuple, a lightweight version of some
# sort of job specification data structure
# This will also be a good place to include code to write out tabulation
# status somehow.
def parse_csv_shim(tup):
    path, name = tup
...
def extract_csv(logpath, outfile, write_status=None):
    ...
    tasks = [(logpath, filename) for filename in files]
    # Do a map over the task list, in parallel over NUM_CPUS processes
    r = pool.map(parse_csv_shim, tasks)
    # Flatten the 2d list
